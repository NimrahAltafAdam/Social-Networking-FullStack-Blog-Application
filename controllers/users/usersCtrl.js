...
const userProfileCtrl = expressAsyncHandler(async (req, res) => {
  ...
  validateMongodbId(id);
  try {
    const myProfile = await User.findById(id).populate("posts");//STEP45-'.populate("posts")' was added here-NEXT STEP IS TO MOVE TO postCtrl.js to set the ctrl for fetech a post and populate a user
    res.json(myProfile);
  } catch (error) {
    res.json(error);
  }
...
//------------------------------
//STEP 33-Forget token generator
//------------------------------
...
const profilePhotoUploadCtrl = expressAsyncHandler(async (req, res) => {
  ...
  //2.Upload to cloudinary //*You need to handle a prmoise inside a function it is called. it is done by adding await
  ...
  const foundUser = await User.findByIdAndUpdate(
    ...
    {new: true}
  );//NEXT STEP IS TO NOW CREATE POST MODEL CONTROLLER AND ROUTE
  //STEP 43-Remove the picturefrom the folder that have already been uploaded on cloudinary
  fs.unlinkSync(localPath); 
  res.json(foundUser); //create a middleware folder uploads and then add a file profilephotoupload
});
